...
// POST route to handle WiFi configuration publishing
router.post("/wifi/user/pub/publish", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "WiFi configuration published successfully" });
  } catch (error) {
    console.error("Error in WiFi publish route:", error.message);
